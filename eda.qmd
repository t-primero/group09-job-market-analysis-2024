...
# Remote share by state
state_remote = df.groupBy("STATE_NAME","REMOTE_TYPE").count().orderBy(desc("count"))
state_remote.show(30, truncate=False)

# Top states by postings
state_totals = df.groupBy("STATE_NAME").count().orderBy(desc("count"))
state_totals.show(30, truncate=False)

# AI vs non AI counts by state
ai_state = df.groupBy("STATE_NAME").agg(_sum("IS_AI_ROLE").alias("ai_posts"), (count("*") - _sum("IS_AI_ROLE")).alias("non_ai_posts"))
ai_state.orderBy(desc("ai_posts")).show(30, truncate=False)
...
if "SALARY" in df.columns:
    ...
    sal_by_state_ai = df.groupBy("STATE_NAME","IS_AI_ROLE").agg(avg("SALARY").alias("avg_salary")).orderBy(desc("avg_salary"))
